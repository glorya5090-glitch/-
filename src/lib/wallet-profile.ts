    ...
    for (const builtinChainProfile of Object.values(tokenProfile.chains ?? {})) {
      ...
      if (matchesBuiltinAlias) {
        return {
          ...
          chainProfile: builtinChainProfile,
        };
      }
    }
  }
...
function collectWalletBalanceTargets(config: WlfiConfig): WalletBalanceTarget[] {
  ...
  const builtinNativeTargets = Object.entries(config.chains ?? {})
    .flatMap(([chainKey, chainProfile]) => {
  ...
  return [...configuredTargets, ...builtinNativeTargets]
    .sort(
      (left, right) =>
