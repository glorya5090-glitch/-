...
test('runAdminUninstallCli removes the global npm cli when invoked from that install', async () => {
  await withIsolatedHome(async ({ homeDir, agentpayHome, rustBinDir, toolDir }) => {
    const uninstallScriptPath = path.join(rustBinDir, 'uninstall-user-daemon.sh');
    const sudoScriptPath = path.join(toolDir, 'sudo');
    const npmScriptPath = path.join(toolDir, 'npm');
    const globalRoot = path.join(homeDir, 'lib', 'node_modules');
    const globalPackageDir = path.join(globalRoot, '@worldlibertyfinancial', 'agentpay-sdk');
    const uninstallLogPath = path.join(homeDir, 'npm-uninstall.log');

    fs.mkdirSync(path.join(globalPackageDir, 'dist'), { recursive: true, mode: 0o700 });
    fs.writeFileSync(
      path.join(globalPackageDir, 'package.json'),
      `${JSON.stringify({ name: '@worldlibertyfinancial/agentpay-sdk', version: '0.1.0' }, null, 2)}\n`,
      { mode: 0o600 },
    );
    fs.writeFileSync(path.join(globalPackageDir, 'dist', 'cli.cjs'), 'module.exports = {};\n', {
      mode: 0o600,
    });
    ...
    writeExecutable(
      ...
      [
        ...
        `if [ "$1" = "uninstall" ] && [ "$2" = "--global" ] && [ "$3" = "@worldlibertyfinancial/agentpay-sdk" ]; then`,
        `  printf '%s\\n' "$@" > "${uninstallLogPath}"`,
        `  rm -rf "${globalPackageDir}"`,
        '  exit 0',
        'fi',
    ...
    process.argv[1] = path.join(globalPackageDir, 'dist', 'cli.cjs');
    process.env.HOME = homeDir;
    process.env.AGENTPAY_HOME = agentpayHome;
    process.env.PATH = `${toolDir}:${originalPath ?? ''}`;

    try {
      const reset = await import(`${modulePath.href}?case=${Date.now()}-run-cli-uninstall-global-npm`);
      await withMockedPrompt('UNINSTALL', async () => {
        await reset.runAdminUninstallCli([]);
      });
    } finally {
    ...
    assert.equal(fs.existsSync(globalPackageDir), false);
    assert.equal(
      fs.readFileSync(uninstallLogPath, 'utf8').trim(),
      'uninstall\n--global\n@worldlibertyfinancial/agentpay-sdk',
    );
    const output = stdoutChunks.join('');
    assert.match(output, /legacy global npm AgentPay SDK CLI removed/u);
    assert.match(output, /legacy global npm package removed from:/u);
    assert.match(output, /reinstall with `curl -fsSL https:\/\/wlfi\.sh \| bash` or use the repo source install steps/u);
  });
});
